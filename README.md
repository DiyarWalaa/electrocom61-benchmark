...
```
electrocom61/
├── data/                        <- partly git-ignored; see the table below
│   ├── Metadata_ElectroCom61.csv          <- you unpack; ships inside v2
│   ├── ElectroCom-61_v2/                  <- you unpack; doi:10.17632/6scy6h8sjz.2
│   │   ├── data.yaml
│   │   ├── README.roboflow.txt
│   │   ├── train/   (images/ + labels/)
│   │   ├── valid/   (images/ + labels/)
│   │   └── test/    (images/ + labels/)
│   ├── v1/                                <- you unpack; doi:10.17632/6scy6h8sjz.1
│   │   └── ElectroCom61 A Multiclass Dataset for Detection of Electronic Components/
│   │       ├── ElectroCom61/
│   │       └── Metadata_ElectroCom61.csv
│   ├── ElectroCom-61_corrected/           <- build_corrected_dataset.py writes this
│   ├── config_provenance.csv              <- ALREADY IN GIT
│   ├── master_results.csv                 <- ALREADY IN GIT
│   ├── latency_by_arch.csv                <- ALREADY IN GIT
│   └── kaggle/                            <- ALREADY IN GIT (small text only)
├── scripts/
├── runs/
...
### What under `data/` is in git

`data/` is **not** wholly git-ignored. `.gitignore` excludes `data/*` and then
re-includes four things, so a fresh clone arrives with `data/` already partly
populated: **49 tracked files, about 217 KB.** What you download is the rest.

| Path | In git? | Why |
|---|---|---|
| `Metadata_ElectroCom61.csv` | no | ships inside the dataset archives |
| `ElectroCom-61_v2/`, `v1/` | no | large, publicly archived, you download them |
| `ElectroCom-61_corrected/` | no | generated by `build_corrected_dataset.py` |
| `config_provenance.csv` | **yes** | a reading of the prior papers, not a measurement — nothing can regenerate it |
| `master_results.csv` | **yes** | built by `collect_results.py`; the backbone of every accuracy figure |
| `latency_by_arch.csv` | **yes** | built by `latency_by_arch.py`; carries the repeatability statement the latency claims rest on |
| `kaggle/` small text | **yes** | eleven per-run `results_*.json` plus `results_latency_unified.json`, each run's `args.yaml`, per-class and training-curve CSVs, and `latency_history/pass2.json` — the evidence behind every number in the paper |
| `kaggle/` binaries | no | ~240 MB of plots, batch mosaics, checkpoints and zips that no result depends on |

The practical consequence: **you do not need to retrain anything to rebuild the
tables and figures.** Their inputs are committed. You need the dataset only to
re-run the audit scripts, which read the images and labels directly.
...
Run from the repository root. No script takes required arguments.

**The audit scripts below are order-independent** — each reads `data/` directly
and writes its own timestamped folder under `runs/`. Run them in any order.

**The paper's generators are not.** `make_tables.py` and `make_figures.py` sit at
the end of a four-stage chain, and each stage consumes the previous stage's
output:

```bash
python scripts/collect_results.py   # data/kaggle/*.json      -> data/master_results.csv
python scripts/latency_by_arch.py   # data/master_results.csv -> data/latency_by_arch.csv
python scripts/make_tables.py       # both CSVs + runs/       -> tables/*.tex
python scripts/make_figures.py      # both CSVs + runs/       -> figures/*
```

Order matters here in a way that will not announce itself. The generators read
whatever is on disk; there is no freshness check tying `master_results.csv` back
to `data/kaggle/`. An out-of-date CSV therefore yields a stale but perfectly
well-formed table, not an error. (`ec61.load_benchmark_rows` does guard the
table's *contents* — it filters to `inclusion == "benchmark"`, asserts the row
count and rejects duplicate `(model, split_set)` pairs — but nothing checks that
the file was rebuilt.)

Both generators also read six committed run directories, so those need not be
regenerated first:

| Run directory | Read by |
|---|---|
| `runs/20260801_device_split/` | `make_figures.py` (F2) |
| `runs/20260802_class_date_provenance/` | both (T1, T2, F1, F2) |
| `runs/20260804_build_corrected_dataset_02/` | `make_tables.py` (T2) |
| `runs/20260804_burst_aware_split_04/` | `make_tables.py` (T2) |
| `runs/20260804_burst_aware_tau_sweep/` | `make_figures.py` (F4) |
| `runs/20260809_split_v1_vs_v2_by_group_03/` | `make_figures.py` (F2b) |
...
python scripts/consecutive_counter_pairs.py          # sequential shots split across folders?
...
| `consecutive_counter_pairs.py` | For the 189 untimestamped `counter` images, finds every pair whose filename numbers differ by 1 and reports which are split across folders, by relationship, with label-geometry distances. Separates pairs the duplicate detector **never compared** (different class multiset) from those it compared and cleared. Read-only — changes no split. | `runs/<date>_consecutive_counter_pairs/` |
