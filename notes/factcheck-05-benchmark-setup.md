...
# Fact-check: subsection 5.6 (2026-08-12)

`Reproducibility artifacts`, inserted verbatim after 5.5. No cross-references to
convert; the one `\ref{sec:training-configuration}` was already written as a
reference and resolves to Section 5.2.

Unusually, almost every claim here is about the repository itself, so nearly all
of it is directly checkable.

## Verified

| Claim | Verdict |
|---|---|
| each generating run writes a directory recording inputs, content hashes and derived values | PASS -- `ec61.write_config` writes `config.json` with `inputs` hashes; e.g. `runs/20260812_make_figures/config.json` carries five per-source SHA-256 values |
| figures are associated with their run by script hash | PASS -- every config records `script_sha256` and `module_sha256` |
| the corrected split is released as a manifest assigning each image to a partition | PASS -- `runs/20260804_burst_aware_split_04/split_manifest.csv` |
| the seed is fixed and the allocation deterministic | PASS -- seed 20260804 |
| no image is modified, added or removed | PASS -- manifest-only; the dataset builder copies rather than moves |
| each run's configuration snapshot retained | PASS -- `args.yaml` for all ten |
| each run's training curves retained | PASS -- all ten (as `training_curves.csv` for eight, `results.csv` for the two yolov9s runs) |
| the diverged run is retained and marked as such | PASS -- `inclusion=diverged` |
| library versions stated in 5.2 | PASS |

## FAILED -- "The ten trained checkpoints are retained"

No checkpoint is in the repository. `.gitignore` excludes them explicitly:

    data/kaggle/artifacts/**/*.pt
    data/kaggle/artifacts/**/*.pth
    data/kaggle/artifacts/**/*.onnx

`git ls-files` returns no `.pt`, `.pth` or `.onnx`. The weights exist on Kaggle,
not here. The sentence lists checkpoints alongside three artifact classes that
ARE committed -- configuration snapshots, per-class results, training curves --
so a reader will reasonably infer all four are in the repository. Three are.

This is the one claim in 5.6 a reviewer could check in thirty seconds and find
false. Either say where the checkpoints are retained, or drop them from the
list.

## Contested -- "no value in either is entered by hand"

Table T3 is generated by `make_tables.py` from `data/config_provenance.csv`.
Two of that file's four columns, `source` and `citation`, are hand-filled --
`scaffold_config_provenance.py` says so in its own docstring ("citation columns
are left blank to be filled by hand") and prints it on every run.

The sentence is literally defensible: the table IS produced by a script from a
data file. But "no value in either is entered by hand" is the stronger claim a
reader will take away, and for T3's provenance and citation columns it is not
true -- those values were typed by a person, into a file, which a script then
read. The hand entry moved upstream; it did not disappear.

## Incomplete -- "each run's ... per-class results"

Missing for two of the ten runs. `yolov9s_corr` and `yolov9s_pub` have no
`per_class.csv`; their artifacts kept Kaggle's original tree. Per-class AP for
those runs exists only inside `results_yolov9s_*.json`, under
`val/per_class_AP50_95` and `test/per_class_AP50_95`.

So the information is retained for all ten, but not in the form the sentence
implies, and not uniformly. "Per-class results" is defensible; a reader
expecting the same file for every run will not find it.

## Imprecise rationale -- "several directories within seconds of one another"

The mechanism is right, the reason given is not. `ec61.make_run_dir` stamps
directories with `%Y%m%d` -- a DATE, no time at all -- and disambiguates repeats
with `_02`, `_03`. Two renders one second apart and two renders eight hours
apart collide identically. So the problem is not that timestamps are too coarse
to separate rapid renders; it is that the directory name carries no time
component, which is exactly why the script hash is the durable association.

## Licence claim -- supportable, not stated in those words

5.6 says the manifest is released "under the same licence as the source
dataset". README records ElectroCom61 as CC BY 4.0 and says of `runs/`: "the CC
BY attribution to the dataset authors travels with anything derived from their
data". That is the same practical effect. It is not phrased as a licence grant
on the manifest itself, and the repository's own MIT grant explicitly "does not
extend to the data". Worth confirming the intended reading before publication.
