...
no model. The audit finds that a quarter of the annotation effort sits in classes
...
textbook 70/20/10. That aggregate is an accident of offsetting deviations, and
the offsets are large. Relative to a uniform draw, the three train-only groups
withhold 155.0 validation images and 77.5 test images between them. On the
validation side the 28 February group returns a surplus of 169.6, and the five
remaining groups contribute $-0.8$, leaving the aggregate 13.8 validation images
above nominal. On the test side the 3 March group returns a surplus of 76.2, the
28 February group subtracts a further 5.2, and the four near-nominal groups
subtract 0.6, leaving the aggregate 7.1 test images below nominal. Both residuals
are under one percentage point of the 2{,}121 images. This is why the problem is
invisible at the level at which splits are normally checked, and it is the reason
the per-group measurement was made at all.
