...
This section asks which classes the published split can evaluate, and answers it
from the released files alone --- image filenames, label files and the metadata
CSV, using only the Python standard library and no model. Two questions have to
be settled first: whether the shipped metadata is a reliable guide to the archive
it travels in, and whether near-duplicate images straddle the split boundary,
since a correction must preserve whatever the published split already achieves.
The audit finds that more than a quarter of the annotation effort sits in classes
the published split cannot evaluate, that the cause is session allocation rather
than class frequency, and that the metadata shipped with v2 describes v1.
...
which is the reading the ordering first invites. Where a long tail is the
concern, rarity is defined by how many training images a category appears in, and
it arises from the natural frequency of categories in images
\citep{gupta2019lvis}. Nothing of the kind is at work here: the classes that
cannot be evaluated are among the most heavily annotated in the dataset. An
imbalanced dataset produces a smooth decline in test representation as class
frequency falls, and cannot produce a discontinuity between adjacent ranks in
which the more frequent side is the empty one.
...
The consequence is that every mAP figure computed on the published split averages
over roughly 46 classes rather than 61, and the two averages are not comparable
