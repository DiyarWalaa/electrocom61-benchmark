...
Two further quantities behind the negative result are worth stating rather than
leaving in a run directory, and both are visible in
Appendix~\ref{sec:appendix-verification-sheet}. The closest test--train pair
scores 0.0515 against a loosest threshold of 0.05, clearing it by 3\%: the
result is real but the margin is thin, and a threshold chosen slightly more
conservatively would have returned a non-zero count. And 95 cross-split pairs
carrying two annotated boxes or fewer are excluded from the ranking, 23 of them
test--train, because a match between box centres is cheap to achieve by chance
when there are few boxes to match. Excluding them is the conservative direction
for a ranking and the permissive one for a zero, which is why the count is
stated here rather than folded silently into the totals.
