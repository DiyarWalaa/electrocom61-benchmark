...
Run directory: `20260809_split_ratio_by_group_03`
...
| 17 Apr 2024 | 54 | 70.4 | 18.5 | 11.1 | +0.4 | -1.5 | +1.1 | near-nominal |
...
- **near-nominal** (4): `6 Mar 2024`, `9 Mar 2024`, `17 Apr 2024`, `18 Nov 2024`
- **train-only** (3): `19 Feb 2024`, `20 Feb 2024`, `iPhone (no timestamp)`
- **skewed** (2): `28 Feb 2024`, `3 Mar 2024`

`near-nominal` means every cell is within **1.5 images** of the uniform draw — as close as integer arithmetic permits.

### Where the two tolerances disagree

Classifying on percentage points instead (within 1.0 pp) moves these groups, and the reason is group size rather than composition:

- **17 Apr 2024** — by images `near-nominal`, by points `skewed`. It holds 54 images, so one file is worth 1.9 percentage points; its worst cell is 0.8 images from a perfect allocation.

Points punish a small session for rounding it cannot avoid, which is why the images measure is the one used for the `shape` column.

### Exact allocations

- **18 Nov 2024** splits 35/10/5 of 50 — exactly 70/20/10, zero deviation in every cell.
