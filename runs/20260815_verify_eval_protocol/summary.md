...
Run directory: `20260815_verify_eval_protocol`

## 1. Ten runs, twenty evaluations

`master_results.csv` has **11 rows**. Every row carries mAP@50 and mAP@50-95 for both validation and test, giving **22 complete evaluations**. The count is taken from the presence of all four figures per run, not from multiplying by two.
...
| published | 45 | 46 | 6 | True |

## 3. ESP32

For each of the 6 published-split runs, the set of classes appearing in the test per-class table minus those appearing in the validation table:

| run | val | test | test - val | val - test |
|---|---|---|---|---|
| `rtdetr_l_pub` | 45 | 46 | ESP32 | (none) |
...
| `rtdetr_l_pub` | train | val | null | 0.7 | 300 |
...
- **conf**: identical across all 11 runs; value(s) `null`
- **iou**: identical across all 11 runs; value(s) `0.7`
- **max_det**: identical across all 11 runs; value(s) `300`
