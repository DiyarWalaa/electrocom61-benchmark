...
Run directory: `20260816_collect_results`

`data/master_results.csv` — 11 rows, 24 columns, from 11 per-run files joined to the unified latency pass on the run slug.
...
| rtdetr-l | published | 0.0039 | 0.0016 | 0.0057 | 0.0026 | 45 | 46 | 19 | 43.7 |
...
The training JSONs' `params` and `gflops` agree with the unified pass's `params_unfused` and `gflops_unfused` on all 10 runs carrying both (GFLOPs to within 0.01, the reporting precision). The two files describe the same weights.
